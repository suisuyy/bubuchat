...
class BingChat {
     ...
     */
    async sendMessage(text, opts = {},onMessage=msg=>{console.log(msg)}) {
        const {
            invocationId = '1',
            onProgress,
            locale = 'en-US',
        ...
        const responseP = new Promise(async (resolve, reject) => {
            ...
            ws.onmessage = (data) => {
                ...
                if(data?.data){
                    let msgObj=JSON.parse(data.data.slice(0,-1))
                    try {
...
async function example() {
    ...
    const res = await api.sendMessage('write js code to edit a object like {a:1,b:{c:3}}',(msgstr)=>{});
    console.log('geted', tes);
}
